...
class ClimbPage extends React.Component {
  ...
  filterComments = (query) =>{
    this.setState({
      comments: this.state.comments.filter(comment => comment.content.includes(query))
    })
  }
    ...
    return (
      <div>
        ...
        <header>
         ...
         <CommentFilter filterComments={this.filterComments} commentSearch={this.commentSearch} />
